...
namespace rsx
{
	...
	void thread::DoState(PointerWrap& p)
	{
		...
		p.Do(thread::stats);
		p.Do(thread::ctrl);
		p.Do(thread::element_push_buffer);
		p.Do(thread::fifo_ret_addr);
		p.Do(thread::framebuffer_status_valid);
		p.Do(thread::m_mtx_task);
		p.Do(thread::m_current_framebuffer_context);
		p.Do(thread::current_fp_metadata);
		p.Do(thread::current_vp_metadata);
